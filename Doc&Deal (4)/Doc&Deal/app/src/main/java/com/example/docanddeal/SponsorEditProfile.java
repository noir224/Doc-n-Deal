...
public class SponsorEditProfile extends AppCompatActivity implements AdapterView.OnItemSelectedListener{
    EditText username, email, phone, description, sponsoredProjects, rejectedProjects, Instagram, Facebook, Twitter;
    Spinner spinner;
    FirebaseAuth fAuth;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sponsoredProjects = findViewById(R.id.SProjectsPerc);
        rejectedProjects = findViewById(R.id.RProjectsPerc);
            ...
            public void onClick(View v) {
                if (username.getText().toString().isEmpty() || email.getText().toString().isEmpty() ||  spinner.getSelectedItem().toString().isEmpty() ) {
                    ...
                    if (validCellPhone(phone.getText().toString()) && validEmail(email.getText().toString())) {
                        ...
                        phoneChanged();
                        descriptionChanged();
                        SponsoredProjectsChanged();
                        RejectedProjectsChanged();
                        instagramChanged();
                        twitterChanged();
                        facebookChanged();
    ...
    private void SponsoredProjectsChanged() {
        if (!sponsoredProjects.getText().toString().equals(SsponsoredProjects)) {
            doc.update("SponsoredProjects", sponsoredProjects.getText().toString());
        }
    }
    private void RejectedProjectsChanged() {
        if (!rejectedProjects.getText().toString().equals(SrejectedProjects)) {
            doc.update("RejectedProjects", rejectedProjects.getText().toString());
        }
    }
